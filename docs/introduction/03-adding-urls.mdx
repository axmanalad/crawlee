...
Previously you've built a very simple crawler that downloads HTML of a single page, reads its title and prints it to the console.
...
:::tip `enqueueLinks` context awareness
...
### Filtering links to specific subdomains

Some websites contain links that finalizes their URL differently from the original context. Users may also like to include subdomains of their choice when enqueuing links. To include subdomains of your choice in your crawl, use the `strategy` argument with the `allowedSubdomains` argument.

```ts
await enqueueLinks({
    strategy: 'split-hostname',
    allowedSubdomains: ['www']
});
```

By default, `allowedSubdomains` will handle ['www'] subdomains when using the "split-hostname" strategy. The `allowedSubdomains` argument can fulfill multiple subdomains (e.g. ['sub1', 'sub2', ..., subn]) Additionally, `allowedSubdomains` is only applicable to the "split-hostname" strategy to keep the other strategies' functionality.
