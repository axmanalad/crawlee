...
You can also specify which subdomains should be allowed when filtering URLs using the `subdomainAliases` parameter. By default, `www` subdomains are allowed alongside the main domain, so crawling `example.com` will also accept links to `www.example.com` for enqueuing.
